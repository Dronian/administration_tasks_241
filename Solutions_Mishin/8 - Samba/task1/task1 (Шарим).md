...
Попробуем подключиться к нашему настроенному серверу Samba с реальной машины (ноута) и провести там некоторые манипуляции.

1. Изменили параметры сети для виртуалки в UTM (Сеть с мостом).

![](7.1.png)

2. Узнали нужный IP-адрес

![](7.2.png)

3. Убеждаемся в том, что Samba работает

![](7.3.png)

4. Подключаемся к серверу Samba в качестве гостя

![](7.4.1.png)

Зайдем в папку public_share. 

![](7.4.2.png)

Попробуем там создать какую-нибудь папку.

![](7.4.3.png)

Видно, что нам не дают этого сделать даже на уровне интерфейса (на первом скрине, папка public_share на сервере Samba – папку создать не можем; на втором, корневая директория моего пользователя в маке – папку создать можем).

5. Подключаемся к серверу Samba в качестве зарегистрированного пользователя

![](7.5.1.png)

Подключение прошло успешно (скриншот выше). Зайдём теперь в папку private_share и попробуем там что-нибудь поделать.

![](7.5.2.png)

Попробуем создать какую-нибудь папку. Видим, что для этого пользователя мы имеем право создавать папки (на уровне интерфейса это подчеркивается).

![](7.5.3.png)

Успешно создали папку.
